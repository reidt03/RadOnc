compareStructures <- function(structures, method=NULL, hausdorff.method=NULL, verbose=TRUE, plot=TRUE, pixels=100, dgrid=NULL) {
...
manEMD <- function(structure1, structure2, doseGrid){ 
  ...
  # if(length(structure1)>length(structure2)){
  #   weightAdjustmentRatio <- length(structure1)/length(structure2)
  #   structure1weight <- matrix(c(rep(1, times = length(structure1)/3)))
  #   structure2weight <- matrix(c(rep(weightAdjustmentRatio, times = length(structure2)/3)))
  # }else{
  #   weightAdjustmentRatio <- length(structure2)/length(structure1)
  #   structure1weight <- matrix(c(rep(weightAdjustmentRatio, times = length(structure1)/3)))
